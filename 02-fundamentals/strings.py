...
datum = "12. 12. 2020"
den, mesic, rok = datum.replace(' ', '').split(".")
unix = f"{rok}-{mesic}-{den}"
# unix = f"{datum.split('.')[2].strip()}-{datum.split('.')[1].strip()}-{datum.split('.')[0].strip()}"
print(unix)
...
def pythonFriendlyVariables(input, type):
    '''
    Type:
    snake -> skake case
    camel -> camel case
    '''
    sanitized_input = unidecode.unidecode(input)
    if type == "camel":
        sanitized_input = sanitized_input.title().replace(" ", "")
        return sanitized_input[0].lower() + sanitized_input[1:]
    elif type == "snake":
        return sanitized_input.lower().replace(" ", "_")
    else:
        print("Unknown case type...")

# print(pythonFriendlyVariables("Ahoj jak se máš", "camel"))
# print(pythonFriendlyVariables("Ahoj jak se máš", "snake"))
...
def randomPasswords(amount):
    return [''.join([random.choice(CAPS) for _ in range(3)] + [random.choice(LOWS) for _ in range(3)] + [random.choice(SPECIALS)] + [random.choice(NUMBERS) for _ in range(3)]) for _ in range(amount)]

'''
def randomPasswords(amount):
    parts = [
        (CAPS, 3),        # 3 velká písmena
        (LOWS, 3),        # 3 malá písmena
        (SPECIALS, 1),    # 1 speciální znak
        (NUMBERS, 3)      # 3 čísla
    ]
    return [
        ''.join(random.choice(chars) for chars, count in parts for _ in range(count))
        for _ in range(amount)
    ]
'''

print(randomPasswords(10))